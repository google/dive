...
class MyConsumer : public VulkanConsumer
{
 public:
        ...
        StructPointerDecoder<Decoded_VkImageMemoryBarrier>* pImageMemoryBarriers) override
    {
        ...
        for (const Decoded_VkImageMemoryBarrier& barrier :
             MakeMetaStructSpan(*pImageMemoryBarriers))
        {
            const auto& [command_buffer_node, image_node] = EmitEdge(commandBuffer, barrier.image);
            association.insert(command_buffer_node);
            association.insert(image_node);
        }
    }
