...
//--------------------------------------------------------------------------------------------------
void DataCore::GatherCaptureStats()
{
    m_capture_stats = CaptureStats();  // Reset any previous stats

    std::array<uint64_t, Dive::Stats::kNumStats> &stats_list = m_capture_stats.stats_list;

    size_t                      event_count = m_capture_metadata.m_event_info.size();
    const Dive::EventStateInfo &event_state = m_capture_metadata.m_event_state;
    ...
    for (size_t i = 0; i < event_count; ++i)
    {
        ...
        else if (info.m_type == Dive::EventInfo::EventType::kDraw)
        {
            ...
            if (info.m_num_indices != 0)
                m_capture_stats.event_num_indices.push_back(info.m_num_indices);
            ...
            for (uint32_t v = 0; v < 16; ++v)
            {
                if (event_state_it->IsViewportSet(v))
                {
                    ...
                    m_capture_stats.viewports.push_back(viewport);
                }
            }
                ...
                event_state_it->IsWindowScissorBRXSet() && event_state_it->IsWindowScissorBRYSet())
            {
                ...
                m_capture_stats.window_scissors.push_back(window_scissor);
            }
        }

        for (size_t ref = 0; ref < info.m_shader_references.size(); ++ref)
            if (info.m_shader_references[ref].m_shader_index != UINT32_MAX)
                m_capture_stats.shader_ref_set.push_back(info.m_shader_references[ref]);
    }

    stats_list[Dive::Stats::kNumBinnigPasses] = m_capture_stats.num_binning_passes;
    stats_list[Dive::Stats::kNumTilingPasses] = m_capture_stats.num_tiling_passes;

    if (!m_capture_stats.event_num_indices.empty())
    {
        GATHER_TOTAL_MIN_MAX_MEDIAN(m_capture_stats.event_num_indices, Indices);
    }

    std::vector<size_t>   shaders_num_instructions;
    std::vector<uint32_t> shaders_num_gprs;

    stats_list[Dive::Stats::kShaders] = m_capture_metadata.m_shaders.size();

    for (const Dive::ShaderReference &ref : m_capture_stats.shader_ref_set)
    {
        ...
        const Dive::Disassembly &disass = m_capture_metadata.m_shaders[ref.m_shader_index];
...
//--------------------------------------------------------------------------------------------------
void DataCore::PrintCaptureStats(std::ostream &ostream)
{
    std::array<uint64_t, Dive::Stats::kNumStats> &stats_list = m_capture_stats.stats_list;
    ...
    for (const Viewport &vp : m_capture_stats.viewports)
    {
    ...
    /*ostream << window_scissor_stats_desc[kWindowScissors_NumBinningPasses] << ": "
            << m_capture_stats.num_binning_passes << "\n";
    ostream << window_scissor_stats_desc[kWindowScissors_NumTilingPasses] << ": "
            << m_capture_stats.num_tiling_passes << "\n";*/

    for (const WindowScissor &ws : m_capture_stats.window_scissors)
    {
...
//--------------------------------------------------------------------------------------------------
const CaptureStats &DataCore::GetCaptureStats() const
{
    return m_capture_stats;
}
