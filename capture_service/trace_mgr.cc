...
std::string TraceManager::TraceState2Str(TraceManager::TraceState state)
{
    switch (state)
    {
...
std::string TraceManager::TraceMode2Str(TraceManager::TraceMode mode)
{
    switch (mode)
    {
...
void TraceManager::OnNewFrame() ABSL_LOCKS_EXCLUDED(m_state_lock)
{
...
absl::Status TraceManager::TriggerTrace() ABSL_LOCKS_EXCLUDED(m_state_lock)
{
    absl::MutexLock lock(&m_state_lock);
...
absl::StatusOr<bool> TraceManager::IsTraceFinished() ABSL_LOCKS_EXCLUDED(m_state_lock)
{
    absl::MutexLock lock(&m_state_lock);
...
absl::StatusOr<std::string> TraceManager::GetFinishedTracePath() ABSL_LOCKS_EXCLUDED(m_state_lock)
{
    absl::MutexLock lock(&m_state_lock);
...
void TraceManager::ResetTrace() ABSL_LOCKS_EXCLUDED(m_state_lock)
{
    absl::MutexLock lock(&m_state_lock);
    LOG(INFO) << "TraceManager::ResetTrace()";
    m_curr_trace_settings = std::nullopt;
    m_state = TraceState::Idle;
...
void TraceManager::ResetAppSession() ABSL_LOCKS_EXCLUDED(m_state_lock)
{
    absl::MutexLock lock(&m_state_lock);
    LOG(INFO) << "TraceManager::ResetAppSession()";
...
void TraceManager::SetFrameConfig(const TraceManager::TraceByFrameConfig config)
{
    m_frame_config = config;
}

void TraceManager::SetDurationConfig(const TraceManager::TraceByDurationConfig config)
{
    m_duration_config = config;
}

bool TraceManager::IsState(TraceState state) ABSL_LOCKS_EXCLUDED(m_state_lock)
{
    absl::MutexLock lock(&m_state_lock);
    if (state != m_state)
    {
...
absl::StatusOr<bool> TraceManager::TraceShouldEnd() ABSL_EXCLUSIVE_LOCKS_REQUIRED(m_state_lock)
{
...
absl::Status TraceManager::ProcessState() ABSL_EXCLUSIVE_LOCKS_REQUIRED(m_state_lock)
{
    switch (m_state)
    {
        case TraceState::Triggered:
