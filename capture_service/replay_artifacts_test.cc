...
MATCHER_P(GfxrReplayArtifactsPathsEq, expected, "")
{
...
TEST(ValidateGfxrReplaySettingsTest, BasicPass)
{
    ...
    GfxrReplayArtifactsPaths expected_res = {};
    ...
    EXPECT_THAT(GetGfxrReplayArtifactsPathsLocal(parent_dir, gfxr_stem),
                IsOkAndHolds(GfxrReplayArtifactsPathsEq(expected_res)));
}
...
TEST(ValidateGfxrReplaySettingsTest, SlashinGfxrStemFail)
{
    ...
    EXPECT_THAT(GetGfxrReplayArtifactsPathsLocal(parent_dir, gfxr_stem).status(),
