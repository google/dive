...
return actual.GetVkCmdName() == expected_name &&
actual.GetVkCmdIndex() == expected_index &&
...
actual->GetSubmitText() == expected_name &&
...
EXPECT_TRUE(AreVulkanCommandInfoEqual(submits[0]->GetVulkanCommands()[1], "vkCmdDraw", 1, nlohmann::ordered_json()));
EXPECT_TRUE(AreVulkanCommandInfoEqual(submits[0]->GetVulkanCommands()[2], "vkCmdDispatch", 2, nlohmann::ordered_json()));

EXPECT_TRUE(AreSubmitInfoEqual(submits[1].get(), "vkQueueSubmit", 1));
ASSERT_EQ(submits[1]->GetVulkanCommands().size(), 3);
EXPECT_TRUE(AreVulkanCommandInfoEqual(submits[1]->GetVulkanCommands()[1], "vkCmdCopyBuffer", 1, nlohmann::ordered_json()));
}
}  // namespace
}  // namespace gfxrecon::decode