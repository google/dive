...
void main(void) {
    ...
    tmp4 = (tmp0 + 1.2) * (tmp1 + 1.4) * (tmp2 * 1.5) * (tmp3 * texture(uTex2D0, tmp3.xy).xyz);
    tmp5 = (tmp0 + 1.6) * (tmp1 + 1.7) * (tmp2 * 1.8) * (tmp3 * 1.9) * (tmp4 * texture(uTex2D0, tmp4.xy).xyz);

    Out[index] = In[index] + uvec3(uMat3 * tmp5 * uMat[gl_GlobalInvocationID.x]);
}
