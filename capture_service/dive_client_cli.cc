...
absl::Status is_capture_directory_busy(Dive::DeviceManager& mgr,
                                       const std::string&   gfxr_capture_directory)
{
    std::string on_device_capture_directory = Dive::kDeviceCaptureDirectory + gfxr_capture_directory;
...
bool retrieve_gfxr_capture(Dive::DeviceManager& mgr, const std::string& gfxr_capture_directory)
{
    ...
    {
        // If the target directory already exists on the local machine, append a number to it to differentiate.
        int counter = 1;
        std::filesystem::path newDirPath;
        while (true) {
            newDirPath = std::filesystem::path(target_download_path.generic_string() + "_" + std::to_string(counter));
    ...
    std::string                 command = "shell ls " + std::string(on_device_capture_directory);
    absl::StatusOr<std::string> output = mgr.GetDevice()->Adb().RunAndGetResult(command);
    ...
    for (const auto& file : file_list) {
        std::string target_file = target_download_path.generic_string() + "/" + file.data();
        auto ret = mgr.GetDevice()->RetrieveTrace(on_device_capture_directory + file.data(), target_file);
                          ...
                          const std::string&   gfxr_capture_directory)
{
    ...
    std::string capture_complete_message = "Capture complete. Press key g+enter to trigger another capture or any other key+enter to stop the application.";
    ...
    std::string on_device_capture_directory = Dive::kDeviceCaptureDirectory + gfxr_capture_directory;
    ret = mgr.GetDevice()->Adb().Run(absl::StrFormat("shell rm -rf %s", on_device_capture_directory));
}
