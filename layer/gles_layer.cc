...
// LAYER modified from https://android.googlesource.com/platform/cts/+/master/hostsidetests/gputools/layers/jni/glesLayer.cpp
#if defined __ANDROID__
extern "C" { 
...
EGLAPI EGLBoolean EGLAPIENTRY glesLayer_eglSwapBuffers(EGLDisplay display,
                                                       EGLSurface surface) {

    frame_num++;
    const char *msg =
        "glesLayer_eglSwapBuffers called in glesLayer" xstr(LAYERNAME);
    ALOGI("%s", msg);
    ...
    typedef EGLBoolean (*PFNEGLSWAPBUFFERSPROC)(EGLDisplay display,
                                                EGLSurface surface);
