...
def get_file_list(file_iter, dive_root, relative_to_dive_root=False):
    """Return a list of files that requires cpplint."""
    ...
    for line in file_iter:
        ...
        if relative_to_dive_root:
            path = dive_root.joinpath(path)
...
def auto_file_list(base_commit, dive_root):
    command = ["git", "-C", str(dive_root), "diff", base_commit, "--name-only"]
    result = subprocess.run(command,
                            capture_output=True, text=True, check=True)
    return get_file_list(result.stdout.splitlines(),
                         dive_root, relative_to_dive_root=True)


def main(args):
    dive_root = pathlib.Path(args.dive_root)

    files = []
    if args.all:
        files = list_files_pathlib_rglob(DIVE_SOURCE_DIRS, dive_root)
    elif args.git_diff_file:
        files = get_file_list_from_diff_file(
            pathlib.Path(args.git_diff_file), dive_root)
    else:
        files = auto_file_list(args.base_commit, dive_root)
    if not files:
...
def parse_args():
    """Parse args for dive_cpplint_wrapper."""
    ...
    file_selection_group.add_argument("--git_diff_file")
    file_selection_group.add_argument("--all", action="store_true")
