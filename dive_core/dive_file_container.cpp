...
public:
    ...
    bool ExtractFileTo(std::filesystem::path path)
    {
        ...
        {
            size_t write_size = std::min<size_t>(512, remain);
            m_stream.read(m_buf, 512);  // Tar does 512 block.
            outfile.write(m_buf, write_size);
            remain -= write_size;
        }
                                                          ...
                                                          DiveVirtualFilesystem& vfs)
{
    ...
    while (reader.Next())
    {
        ...
        DiveVirtualFilesystem::File file_info;
        file_info.m_name = GetExtractedName(name);
        file_info.m_size = size;
        file_info.m_disk_path = archive_path;
        file_info.m_offset = reader.GetOffset();
        file_info.m_extract_path = out_path / GetExtractedName(name);
