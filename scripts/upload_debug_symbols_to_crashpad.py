...
def write_step(message):
    print(f"[Upload debug symbols] {message}")
...
def main():
    ...
    parser.add_argument("--sym-file-path", required=True, help="Path to the .sym file")
    parser.add_argument("--upload-url", required=True, help="Base URL for the symbol collector")
    ...
    if args.config != BuildType.REL_WITH_DEB_INFO:
        write_step(f"Skipping upload debug symbols for configuration: {args.config}. Only {BuildType.REL_WITH_DEB_INFO} is supported.")
        return
    ...
    try:
        ...
        write_step("Success: Debug symbols uploaded and finalized")

    except urllib.error.HTTPError as e:
        try:
            write_step(f"Response context: {e.read().decode('utf-8')}")
        except Exception:
            pass
        raise
