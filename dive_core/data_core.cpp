...
//--------------------------------------------------------------------------------------------------
bool DataCore::ValidateLRZ(const std::string &output_file_name)
{
    std::unique_ptr<EmulateStateTracker> state_tracker(new EmulateStateTracker);
    ...
    if (!lrz_validator.ProcessSubmits(m_capture_data.GetSubmits(),
                                      m_capture_data.GetMemoryManager()))
    {
        return false;
    }
...
// LRZValidator
// =================================================================================================
LRZValidator::LRZValidator(EmulateStateTracker &state_tracker, std::ofstream &output_file) :
    m_state_tracker(state_tracker),
...
//--------------------------------------------------------------------------------------------------
void LRZValidator::OnSubmitStart(uint32_t submit_index, const SubmitInfo &submit_info)
{
    m_state_tracker.Reset();
    m_skip_drawcalls = false;
}
...
//--------------------------------------------------------------------------------------------------
void LRZValidator::OnSubmitEnd(uint32_t submit_index, const SubmitInfo &submit_info) {}
...
//--------------------------------------------------------------------------------------------------
bool LRZValidator::OnPacket(const IMemoryManager &mem_manager,
                            uint32_t              submit_index,
                            uint32_t              ib_index,
                            uint64_t              va_addr,
                            Pm4Type               type,
                            uint32_t              header)
{
    ...
    {
        std::string draw_string = Util::GetEventString(mem_manager, submit_index, va_addr, opcode);
