...
void TraceDialog::OnStartClicked()
{
    ...
    else
    {
        ...
        if (m_gfxr_capture)
        {
            ...
            std::string on_device_capture_directory = Dive::kDeviceCaptureDirectory + m_gfxr_capture_file_directory_input_box->text().toStdString();
...
absl::StatusOr<int64_t> GfxrCaptureWorker::getGfxrCaptureDirectorySize(Dive::AndroidDevice *device)
{
    ...
    std::string                 ls_command = "shell ls " + std::string(m_capture_path);
    ...
    for (std::string file : m_file_list) {
        std::string command = "shell ls -l " + m_capture_path + file.data() + " | awk '{print $5}'";
...
void GfxrCaptureWorker::run()
{
    ...
    for (std::string file : m_file_list) {
        std::string target_file = m_target_capture_path + file.data();
        auto ret = device->RetrieveTrace(m_capture_path + file.data(), target_file);

        if (!ret.ok())
        {
            std::cout << "Failed to retrieve gfxr capture: " << ret.message() << std::endl;
            qDebug() << ret.message().data();
            ShowErrorMessage(ret.message().data());
            return;
        }
...
void TraceDialog::RetrieveGfxrCapture()
{
    ...
    workerThread->SetGfxrTargetCapturePath(m_gfxr_capture_file_local_directory_input_box->text()
                                       .toStdString());
