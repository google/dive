...
function run_copyright_headers() {
  ...
  for suffix in "cc" "cpp" "frag" "glsl" "go" "h" "hpp" "java" "js" "sh" "vert" "xml"; do
    ...
    git diff origin/main HEAD --name-only --| grep "\.${suffix}$" | grep -v "^third_party/" | grep -E -v '^prebuild/*' | xargs grep -L "Copyright .* Google"
...
function run_clang_format() {
  ...
  git diff origin/main HEAD --name-only --diff-filter=ACMR --| grep -E "\.(c|cpp|h|hpp|cc)$" | grep -E -v '^third_party/*' | grep -E -v '^prebuild/*' | xargs $CLANG_FORMAT -i -style=file
}
