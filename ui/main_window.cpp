...
bool MainWindow::LoadGfxrFile(const char *file_name)
{
    ...
    // Only create the gfxr shortcuts once.
    if (!m_search_gfxr_vulkan_command_arguments_shortcut)
    {
        CreateGfxrShortcuts();
    }
...
//--------------------------------------------------------------------------------------------------
void MainWindow::CreateDefaultShortcuts()
{
    // Search Shortcut
...
//--------------------------------------------------------------------------------------------------
void MainWindow::CreateGfxrShortcuts()
{
    // Gfxr Vulkan Command Arguements Search Shortcut
    m_search_gfxr_vulkan_command_arguments_shortcut = new QShortcut(QKeySequence(
                                                                    SHORTCUT_COMMANDS_SEARCH),
                                                                    this);
    connect(m_search_gfxr_vulkan_command_arguments_shortcut, &QShortcut::activated, [this]() {
...
{
    m_search_shortcut->setEnabled(false);
    m_search_commands_shortcut->setEnabled(false);
    m_overview_tab_shortcut->setEnabled(false);
    m_command_tab_shortcut->setEnabled(false);
    m_shader_tab_shortcut->setEnabled(false);
    m_event_state_tab_shortcut->setEnabled(false);

    if (m_search_gfxr_vulkan_command_arguments_shortcut)
    {
        m_search_gfxr_vulkan_command_arguments_shortcut->setEnabled(false);
    }
}

//--------------------------------------------------------------------------------------------------
void MainWindow::EnableDefaultShortcuts()
{
    m_search_shortcut->setEnabled(true);
    m_search_commands_shortcut->setEnabled(true);
    m_overview_tab_shortcut->setEnabled(true);
    m_command_tab_shortcut->setEnabled(true);
    m_shader_tab_shortcut->setEnabled(true);
    m_event_state_tab_shortcut->setEnabled(true);
}

//--------------------------------------------------------------------------------------------------
void MainWindow::EnableGfxrShortcuts()
{
    m_search_shortcut->setEnabled(true);
    m_search_gfxr_vulkan_command_arguments_shortcut->setEnabled(true);
}
