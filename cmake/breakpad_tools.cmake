...
# Refer to the Build.md prerequisites for instructions on how to install dump_syms.
...
function(upload_debug_symbols TARGET_NAME)
    ...
    add_custom_command(
        ...
        POST_BUILD
        COMMAND
            "$<IF:$<CONFIG:RelWithDebInfo>,$<TARGET_FILE:dump_syms>;${DEBUG_FILE},${CMAKE_COMMAND};-E;true>"
            > "${SYM_FILENAME}"
        WORKING_DIRECTORY "${BINARY_OUTPUT_DIR}"
    ...
    set(UPLOAD_DEBUG_SYMBOLS_SCRIPT_PATH
        "${CMAKE_CURRENT_SOURCE_DIR}/../scripts/upload_debug_symbols_to_crashpad.py"
    )
    add_custom_command(
        TARGET ${TARGET_NAME}
        POST_BUILD
        COMMAND
            "${Python3_EXECUTABLE}" "${UPLOAD_DEBUG_SYMBOLS_SCRIPT_PATH}"
            --sym-file-path "${SYM_FILENAME}" --upload-url
            "${CRASHPAD_UPLOAD_URL}" --config "$<CONFIG>"
        WORKING_DIRECTORY "${BINARY_OUTPUT_DIR}"
        COMMENT
