...
{
    static const absl::NoDestructor<std::map<Command, CommandMetadata>> registry({
    {
    Command::kListDevice,
    {
    .name = "list_device",
...
{
    if (text == "vulkan_openxr")
    {
        *type = AppType::kVulkan_OpenXR;
        return true;
    }
    if (text == "vulkan_non_openxr")
    {
        *type = AppType::kVulkan_Non_OpenXR;
        return true;
    }
    if (text == "vulkan_cli_non_openxr")
    {
    ...
    *error = "unknown value for --type. Valid values: vulkan_openxr, vulkan_non_openxr, "
             "vulkan_cli_non_openxr, gles_openxr";
    return false;
}
...
std::string AbslUnparseFlag(AppType type)
{
    ...
    case AppType::kVulkan_OpenXR:
        return "vulkan_openxr";
    case AppType::kVulkan_Non_OpenXR:
        return "vulkan_non_openxr";
    case AppType::kVulkanCLI_Non_OpenXR:
        return "vulkan_cli_non_openxr";
...
ABSL_FLAG(Command,
          command,
          Command::kNone,
          "The operation to perform. Available commands:\n"
          "\tlist_device  : List connected Android devices.\n"
          "\tlist_package : List installable packages on the selected device.\n"
          "\trun          : Run an app for manual testing or external capture.\n"
          "\tpm4_capture  : Run an app and trigger a PM4 capture after a delay.\n"
          "\tgfxr_capture : Run an app and enable GFXR capture via key-press.\n"
          "\tgfxr_replay  : Deploy and run a GFXR replay.\n"
          "\tcleanup      : Clean up app-specific settings on the device.\n");
...
ABSL_FLAG(AppType,
          ...
          "The type of application being targeted. This determines the setup and capture logic.\n"
          "\tValid values: vulkan_openxr, vulkan_non_openxr, vulkan_cli_non_openxr, gles_openxr");
...
absl::Status InternalRunPackage(const CommandContext& ctx, bool enable_gfxr)
{
    ...
    case AppType::kVulkanCLI_Non_OpenXR:
        ...
        break;
    case AppType::kGLES_OpenXR:
        return absl::InvalidArgumentError("GLES OpenXR not yet supported in SetupApp logic.");
    default:
        // Use AbslUnparseFlag to convert enum back to string for the error message
        return absl::InvalidArgumentError("Unknown app type: " +
                                          AbslUnparseFlag(ctx.options.app_type));
    }
