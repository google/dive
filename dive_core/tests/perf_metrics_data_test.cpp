...
protected:
    void SetUp() override
    {
        ...
        TEST_DATA_DIR "dive_core/tests/mock_available_metrics.csv");
...
TEST_F(PerfMetricsDataTest, LoadFromCsv)
{
    ...
    LoadFromCsv(TEST_DATA_DIR "dive_core/tests/mock_perf_metrics_data.csv", *m_available_metrics);
    ...
    // Check first record
    Dive::PerfMetricsRecord expected_record1{ .m_context_id = 1,
                                              .m_process_id = 100,
                                              .m_frame_id = 1000,
                                              .m_cmd_buffer_id = 10000,
                                              .m_draw_id = 1,
                                              .m_draw_label = 1,
                                              .m_program_id = 1,
                                              .m_draw_type = 1,
    ...
    // Check second record
    Dive::PerfMetricsRecord expected_record2{ .m_context_id = 2,
                                              .m_process_id = 200,
                                              .m_frame_id = 2000,
                                              .m_cmd_buffer_id = 20000,
                                              .m_draw_id = 2,
                                              .m_draw_label = 2,
                                              .m_program_id = 2,
                                              .m_draw_type = 2,
...
TEST_F(PerfMetricsDataTest, LoadFromCsv_MalformedRowSkipped)
{
    auto perf_metrics_data = Dive::PerfMetricsData::
    LoadFromCsv(TEST_DATA_DIR "dive_core/tests/mock_perf_metrics_data_malformed.csv",
                *m_available_metrics);
    ASSERT_TRUE(perf_metrics_data);
    const auto& records = perf_metrics_data->GetRecords();
...
TEST_F(PerfMetricsDataTest, LoadFromCsv_FailedWithNoHeader)
{
    auto perf_metrics_data = Dive::PerfMetricsData::
    LoadFromCsv(TEST_DATA_DIR "dive_core/tests/mock_perf_metrics_data_no_header.csv",
                *m_available_metrics);
    ASSERT_FALSE(perf_metrics_data);
}

TEST_F(PerfMetricsDataTest, LoadFromCsv_WrongColumnsSkipped)
{
    auto perf_metrics_data = Dive::PerfMetricsData::
    LoadFromCsv(TEST_DATA_DIR "dive_core/tests/mock_perf_metrics_data_wrong_columns.csv",
                *m_available_metrics);
    ASSERT_TRUE(perf_metrics_data);
    const auto& records = perf_metrics_data->GetRecords();
...
TEST(PerfMetricsData, LoadFromCsv_UnknownMeticTypeSkipped)
{
    auto available_metrics = Dive::AvailableMetrics::LoadFromCsv(
    TEST_DATA_DIR "dive_core/tests/mock_available_metrics_unknown_type.csv");
    ASSERT_TRUE(available_metrics);

    auto perf_metrics_data = Dive::PerfMetricsData::
    LoadFromCsv("dive_core/tests/mock_perf_metrics_data_unknown_type.csv", *available_metrics);
    ASSERT_TRUE(perf_metrics_data);

    const auto& records = perf_metrics_data->GetRecords();
    ASSERT_EQ(records.size(), 1);
    ASSERT_EQ(records[0].m_metric_values.size(), 0);
}
}  // namespace
