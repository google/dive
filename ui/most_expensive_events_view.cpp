                                            ...
                                            const QModelIndex& index) const
{
    ...
    if (index.column() == kShaderStageColumn)
    {
        ...
        Dive::ShaderStage stage_order[Dive::kShaderStageCount] = {
            Dive::ShaderStage::kShaderStageVs, Dive::ShaderStage::kShaderStageGs,
            Dive::ShaderStage::kShaderStageHs, Dive::ShaderStage::kShaderStagePs,
        ...
        for (uint32_t i = 0; i < Dive::kShaderStageCount; ++i)
        {
            ...
            if (stage == Dive::ShaderStage::kShaderStageVs) shader_text = "VS";
            if (stage == Dive::ShaderStage::kShaderStageGs) shader_text = "GS";
            if (stage == Dive::ShaderStage::kShaderStageHs) shader_text = "HS";
            if (stage == Dive::ShaderStage::kShaderStagePs) shader_text = "PS";
            if (stage == Dive::ShaderStage::kShaderStageCs) shader_text = "CS";

            QSize text_size = painter
                                  ->boundingRect(QRect(0, 0, 0, 0), Qt::AlignLeft,
    ...
    : m_capture_metadata(capture_metadata)
{
    ...
    QObject::connect(m_event_list, SIGNAL(customContextMenuRequested(QPoint)), this,
