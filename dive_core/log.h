...
{
 public:
    LogEntryBuilder(ILog& log, LogType type, LogCategory cat, CrossRef ref, const char* file,
                    int line);
    LogEntryBuilder(ILog& log, LogType type, LogCategory cat, LogAssociation association,
                    uint32_t id,  // Ignored if (association == kNone)
                    const char* file, int line);
    ~LogEntryBuilder();

    template <typename T>
    LogEntryBuilder& operator<<(const T& val)
    {
        m_buf << val;
        return *this;
    }
...
class LogEntryBuilderNull
{
 public:
    template <typename T>
    LogEntryBuilderNull& operator<<(const T& val)
    {
        return *this;
    }
...
#define LOG_INTERNAL_WARNING(log_stream, cat)                                              \
    LogEntryBuilder(log_stream, LogType::kWarning, cat, LogAssociation::kNone, UINT32_MAX, \
                    __FILE__, __LINE__)

#define LOG_INTERNAL_ERROR(log_stream, cat)                                                        \
    LogEntryBuilder(log_stream, LogType::kError, cat, LogAssociation::kNone, UINT32_MAX, __FILE__, \
                    __LINE__)
#else
...
#define LOG_PUBLIC_INFO(log_stream, cat)                                                      \
    Dive::LogEntryBuilder(log_stream, Dive::LogType::kInfo, cat, Dive::LogAssociation::kNone, \
                          UINT32_MAX, __FILE__, __LINE__)
...
#define LOG_PUBLIC_EVENT_WARNING(log_stream, cat, id)                                         \
    LogEntryBuilder(log_stream, LogType::kWarning, cat, LogAssociation::kEvent, id, __FILE__, \
                    __LINE__)
#define LOG_PUBLIC_EVENT_ERROR(log_stream, cat, id)                                         \
    LogEntryBuilder(log_stream, LogType::kError, cat, LogAssociation::kEvent, id, __FILE__, \
                    __LINE__)

// Warnings/Errors associated with a specific Barrier
#define LOG_PUBLIC_BARRIER_WARNING(log_stream, cat, id)                                         \
    LogEntryBuilder(log_stream, LogType::kWarning, cat, LogAssociation::kBarrier, id, __FILE__, \
                    __LINE__)
#define LOG_PUBLIC_BARRIER_ERROR(log_stream, cat, id)                                         \
    LogEntryBuilder(log_stream, LogType::kError, cat, LogAssociation::kBarrier, id, __FILE__, \
                    __LINE__)

#define LOG_GFR_WARNING(log_stream, cat, id)                                                     \
    LogEntryBuilder(log_stream, LogType::kWarning, cat, CrossRef(LogAssociation::kGFRIndex, id), \
                    __FILE__, __LINE__)
